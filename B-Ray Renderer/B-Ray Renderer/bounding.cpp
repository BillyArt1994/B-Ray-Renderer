...
//-----------------------BoxBounding-----------------------
BoxBounding::BoxBounding(std::vector<Vertex>& _vert_list) {
	Vec3 minpoint_(FLT_MAX), maxpoint_(FLT_MIN);
	for (const Vertex& i : _vert_list)
	{
...
bool BoxBounding::Contains(const BoxBounding& boxBound) const {
...
bool BoxBounding::Contains(const Vec3& point) const {
...
bool BoxBounding::IntersectRay(const Ray& r, float& t) const {
...
//-----------------------SphereBounding-----------------------
SphereBounding::SphereBounding(std::vector<Vertex>& _vert_list) {
	Vec3 minpoint_(FLT_MAX), maxpoint_(FLT_MIN);
	for (const Vertex& i : _vert_list)
	{
		minpoint_.x = Min(i.position.v[0], minpoint_.x);
...
bool SphereBounding::IntersectRay(const Ray& r, float& t) const {
	return true;
}

bool SphereBounding::Contains(const BoxBounding& boxBound)const {
